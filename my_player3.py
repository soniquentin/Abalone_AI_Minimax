...
class MyPlayer3(PlayerAbalone):
    ...
    params = {}
    #Change le hasard
    for param in param_tuning["current_test"].keys():
        params[param] = random.choice(param_tuning["range"])
    ...
    def heuristic(self, current_state: GameState) -> float:
        ...
        for i, (coord, marble) in enumerate(current_state.get_rep().get_env().items()):
            ...
            if maxi_cube_coord == 4:
                ...
                for a,b,c in direction_to_check :
                    ...
                    while color_cur == color_in_progress :
                        color_edge_count += 1
                        cur += 1
                        piece_cur = current_state.get_rep().find( MyPlayer3.to_coord.get( (x+a*cur, y+b*cur, z+c*cur) , -1 ) )
                        if piece_cur == -1 :
                            break
                        else :
            ...
            for x_n, y_n, z_n in neighbours_cube_coord:
                neighbour_coord = MyPlayer3.to_coord.get( (x_n, y_n, z_n) , -1) 
            ...
            for j, (coord2, marble2) in enumerate(current_state.get_rep().get_env().items()):
                ...
                if color_marble == color_marble2 :
                    x2,y2,z2 = MyPlayer3.to_cube[coord2]
        ...
        return MyPlayer3.params["diff"]*diff + MyPlayer3.params["edge"]*edge + MyPlayer3.params["center"]*center + MyPlayer3.params["cohesion_diff"]*cohesion_diff + MyPlayer3.params["threat_diff"]*threat_diff + MyPlayer3.params["density_diff"]*density_diff
